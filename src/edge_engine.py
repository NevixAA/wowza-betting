...
# ── model↔market blend ────────────────────────────────────────────────────────
def blend_weight(odds: float, n_eff: int, market: str) -> float:
    """Weight on the MODEL. Small by design (market is the baseline). Shrinks further on
    long odds, low sample, and rare-event markets. Given AUC_marginal≈0.5, cap is low."""
    b = _band(odds)
    if b is None:
        return 0.0
    w = min(0.30, b["max_w"] * 0.6)                 # global model-weight ceiling 0.30
    if n_eff < b["min_n"]:
        w *= n_eff / max(1, b["min_n"])
    if market in RARE_EVENT_MARKETS:
        w *= 0.5
    return max(0.0, min(0.30, w))
...
def classify(c: Candidate) -> dict:
    ...
    w = blend_weight(best, c.n_eff, c.market)
